...
def select_nodes(building_x, building_y, interval_dist=1):
  node_x =  []
  node_y =  []
...
if __name__ == '__main__':
  ...
  #xml = './map/nyc_small.osm'
  #xml = './map/nyc_medium.osm'
  #xml = './map/nyc_big.osm'
  #xml = 'seattle_u_washington.osm'
  #xml = './map/dallas_11.osm'
  #xml = './map/nyc_small_area.osm'
  #xml = './small_maps/dallas_olive_avenue.osm'
  #xml = './map/new_orleans_small.osm'
  xml = '1.osm'
  ...
  building_x, building_y, max_x, max_y =\
       transform_to_cartesian_coord(building_lat =  building_lat, 
                               ...
                               ref_lat = min_lat,
                               ref_lon = min_lon)
  print "bx,by",max(building_x),max(building_y)
  del building_lat, building_lon #save memory
  node_x, node_y = select_nodes(building_x, building_y, interval_dist = interval_dist ) 
  total_nodes = len(node_x)
  ...
  calculate_LOS(buildin_x = building_x,
                ...
                bin_LOS =  bin_LOS,
                file_name =  xml+".edge.txt")
  
  print "Debug: saving the file data"
  pkl_data ={}
  pkl_data["building_x"] = building_x
  pkl_data["building_y"] = building_y
  pkl.dump(obj = pkl_data, file = open(xml+".building_temp.pkl",'wb'))
  
  del pkl_data
  
  pkl_data = {}
  pkl_data["node_x"] = node_x
  pkl_data["node_y"] = node_y
  pkl.dump(obj = pkl_data, file = open(xml+".node_temp.pkl",'wb'))
  
  del pkl_data
  print" DEBUG: Done...."

  #generate_visual_map( building_x, building_y, node_x, node_y , adj = None)
